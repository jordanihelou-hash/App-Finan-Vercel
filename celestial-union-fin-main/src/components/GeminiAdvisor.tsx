...
export const GeminiAdvisor = forwardRef<GeminiAdvisorHandle, GeminiAdvisorProps>(
  function GeminiAdvisorInner(
    ...
    const [analysis, setAnalysis] = useState<Analysis>(() =>
      localScore(income, paidExpense, forecastExpense, invested)
    );
    const [loading, setLoading] = useState(false);
    const [apiError, setApiError] = useState<string | null>(null);
    const [overBudgetBanner, setOverBudgetBanner] = useState<string[] | null>(null);

    // Atualiza score local quando os dados chegam pela primeira vez
    useEffect(() => {
      setAnalysis((prev) => {
        if (
          prev.score === 0 &&
          (income > 0 || paidExpense > 0 || forecastExpense > 0 || invested > 0)
        ) {
          return localScore(income, paidExpense, forecastExpense, invested);
        }
        return prev;
      });
    }, [income, paidExpense, forecastExpense, invested]);
